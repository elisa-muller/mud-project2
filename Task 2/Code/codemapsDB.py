...
class Codemaps :
    ...
    #--------- Load DrugBank entries and create token set for orthographic features
    def __load_drugbank(self, drugbank_file):
        allowed_types = {"drug", "group"}  # ignore brands for now
        ...
        with open(drugbank_file, "r", encoding="utf-8", errors="ignore") as f:
            for line in f:
                ...
                # FILTERS
                # 1. avoid things that start with a number
                if name[0].isdigit():
                    continue

                # 2. remove commercial noise
                noisy_words = ["spf", "foundation", "moisturizer", "sunscreen",
                            "cream", "lotion", "lip", "gel", "spray"]
                if any(w in name for w in noisy_words):
                    continue
        ...
        print("Filtered DrugBank entries:", len(self.drugbank_entries))
