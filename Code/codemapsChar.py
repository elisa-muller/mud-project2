    ...
    def save(self, name) :
        with open(name+".idx","w") as f :
            ...
            for key in self.char_index :
                print('CHAR', key, self.char_index[key], file=f)
    ...
    def encode_words(self, data) :
        ...
        for s in data.sentences():
            ...
            for w in s:
                ...
                sent_feats.append([is_capitalized, is_all_caps, is_all_lower,
                                   has_digit, has_hyphen, has_slash, has_dot,
                                   is_alnum_mixed])
        ...
        # character-level encoding: (n_sentences, maxlen, maxcharlen)
        Xc = []
        for s in data.sentences():
            sent_chars = []
            for w in s:
                char_ids = [self.char_index.get(c, self.char_index['UNK'])
                            for c in w['lc_form'][:self.maxcharlen]]
                char_ids += [self.char_index['PAD']] * (self.maxcharlen - len(char_ids))
                sent_chars.append(char_ids)
            Xc.append(sent_chars)

        padded_Xc = []
        for sent_chars in Xc:
            if len(sent_chars) < self.maxlen:
                sent_chars = sent_chars + [[self.char_index['PAD']]*self.maxcharlen] * (self.maxlen - len(sent_chars))
            else:
                sent_chars = sent_chars[:self.maxlen]
            padded_Xc.append(sent_chars)
        Xc = np.array(padded_Xc, dtype=np.int32)

        return [Xw, Xs, Xlw, Xp, Xf, Xc]
